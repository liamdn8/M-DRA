...
def validate_jobs(jobs_path):
    ...
    if (jobs["start_time"] < 0).any():
        print("❌ ERROR: start_time must be non-negative")
        return False
...
def validate_resource_capacity(dataset_path, strict=False, cpu_threshold=0.9, mem_threshold=0.8):
    """
    Validate that cluster capacities can support job demands.
    
    Args:
        dataset_path: Path to dataset directory
        strict: If True, validate temporal constraints per timeslice
        cpu_threshold: Maximum allowed CPU utilization ratio (default 0.9 = 90%)
        mem_threshold: Maximum allowed memory utilization ratio (default 0.8 = 80%)
    """
    ...
    for _, cluster in clusters.iterrows():
        ...
        if cid in cluster_caps["default_cluster"].values:
            ...
            if strict:
                # Strict validation: Check each timeslice
                max_time = (jobs["start_time"] + jobs["duration"]).max()
                
                for t in range(max_time + 1):
                    # Find active jobs at this timeslice
                    ...
                    if len(active_jobs) > 0:
                        ...
                        cpu_ratio = cpu_req_t / caps["cpu_cap"] if caps["cpu_cap"] > 0 else float('inf')
                        mem_ratio = mem_req_t / caps["mem_cap"] if caps["mem_cap"] > 0 else float('inf')
                        vf_ratio = vf_req_t / caps["vf_cap"] if caps["vf_cap"] > 0 else (0 if vf_req_t == 0 else float('inf'))
                        
                        # Check against thresholds
                        if cpu_ratio > cpu_threshold:
                            violations.append(
                                f"Cluster {cid} at t={t}: CPU {cpu_ratio*100:.1f}% > {cpu_threshold*100:.0f}% threshold "
                                f"({cpu_req_t:.1f}/{caps['cpu_cap']:.1f})"
                            )
                        
                        if mem_ratio > mem_threshold:
                            violations.append(
                                f"Cluster {cid} at t={t}: Memory {mem_ratio*100:.1f}% > {mem_threshold*100:.0f}% threshold "
                                f"({mem_req_t:.0f}/{caps['mem_cap']:.0f})"
                            )
                        
                        if vf_ratio > 1.0:
                            violations.append(
                                f"Cluster {cid} at t={t}: VF {vf_ratio*100:.1f}% > 100% "
                                f"({vf_req_t}/{caps['vf_cap']})"
                            )
            else:
                # Non-strict: Check peak demand (worst case: all jobs running simultaneously)
    ...
    if strict and violations:
        print(f"❌ STRICT VALIDATION FAILED: {len(violations)} violations found")
        print(f"   CPU threshold: {cpu_threshold*100:.0f}%, Memory threshold: {mem_threshold*100:.0f}%")
        ...
        if strict:
            print(f"✅ Strict resource capacity validated - all timeslices within thresholds")
            print(f"   CPU ≤ {cpu_threshold*100:.0f}%, Memory ≤ {mem_threshold*100:.0f}%")
        else:
...
def main():
    ...
    parser.add_argument("--strict", action="store_true",
                       help="Enable strict per-timeslice validation")
    parser.add_argument("--cpu-threshold", type=float, default=0.9,
                       help="Maximum CPU utilization threshold for strict mode (default: 0.9)")
    parser.add_argument("--mem-threshold", type=float, default=0.8,
                       help="Maximum memory utilization threshold for strict mode (default: 0.8)")
    ...
    if args.strict:
        print(f"Mode: STRICT (CPU ≤ {args.cpu_threshold*100:.0f}%, Memory ≤ {args.mem_threshold*100:.0f}%)")
    else:
    ...
    if success:
        ...
        success &= validate_resource_capacity(dataset_path, 
                                             strict=args.strict,
